...
function main() {
	...
	// 1.1 Locate Parents
	var $parent = Interface.locateParent();
	...
	// 1.3 Load Interface
	Interface.load($parent, cannedAnalysisData);
...
var Interface = {
	...
	prepare: function(cannedAnalysisData) {
		...
		// Add text
		interfaceHTML = '<div ng-include="template" class="ng-scope"><div class="col-md-offset-1 ng-scope"><h3>Canned Analyses</h3><p>The table below displays the canned analyses associates to the selected dataset, grouped by tool.</p><p>To browse the canned analyses, select a tool from the menu below.  The search bar can be used to restrict the menu to desired tools.  Mouse over the info icon to receive additional information regarding computational tools.</p>' + toolTableHTML + '</div></div>';
...
var eventListener = {
	...
	toggleTab: function(cannedAnalysisData) {
		...
		$('.datasets2tools-tab-link').click(function(evt) {
			...
			// Get Interface
			interfaceHTML = Interface.prepare(cannedAnalysisData);
...
var toolTable = {
	...
	prepare: function(cannedAnalysisData) {
		...
		for (i = 0; i < toolsSorted.length; i++) {
			...
			// Add HTML
			// Tool Column
			toolTableHTML += '<tr><td class=""><img class="datasets2tools-cannedanalysis-link-img" src="' + toolData[toolId]['tool_icon_url'] + '"><div class="">' + toolData[toolId]['tool_name'] + '</div></td>';

			// Tool Description Column
			toolTableHTML += '<td class="">' + toolData[toolId]['tool_description'] + '</td>';

			// Canned Analyses Column
			toolTableHTML += '<td class="">' + Object.keys(cannedAnalyses[toolId]).length + '</td></tr>';
		}

		// Close Table
		toolTableHTML += '</table>'

		console.log(nrOfCannedAnalyses);
		console.log(toolsSorted);
