...
var lincsInterface = {
	...
	prepare: function(cannedAnalysisData) {
		...
		// Add text
		interfaceHTML = '<div class="datasets2tools-browsebar datasets2tools-lincs"><h2 class="datasets2tools-cannedanalyses-label">Analysis Tools</h2><p>The table below displays the analysis tools which have been used to perform canned analyses on selected dataset.</p><p>To browse the canned analyses, select a tool from the menu below.  The search bar can be used to restrict the menu to desired tools.  Mouse over the info icon to receive additional information regarding computational tools.</p>' + toolTableHTML + '</div>';
...
var lincsEventListener = {
	...
	toggleTab: function(cannedAnalysisData) {
		...
		$('.datasets2tools-tab-link').click(function(evt) {
			...
			var $displayTab = $('.datasets2tools-tab-link').parents('.ng-scope').children('.ng-scope').eq(0),
				interfaceHTML = '<div ng-include="template" class="ng-scope"><div class="col-md-offset-1 ng-scope">';

			// Get Interface
			interfaceHTML += lincsInterface.prepare(cannedAnalysisData) + '</div></div>';
	...
	clickPlusButton: function(cannedAnalysisData) {
		...
		$('.ng-scope').on('click', '.datasets2tools-browsebar table tr .datasets2tools-tooltable-plus-button', function(evt) {
			// Define Variables
			var $evtTarget = $(evt.target),
				toolId = $evtTarget.attr('id'),
				$displayTab = $evtTarget.parents('.datasets2tools-browsebar');

			// Prepare Interface
			var interfaceHTML = lincsBrowseTable.prepare(cannedAnalysisData, toolId);
	...
	clickGoBackLink: function(cannedAnalysisData) {

		$('.ng-scope').on('click', '.datasets2tools-browsebar .datasets2tools-lincs-goback-link', function(evt) {
			
			// Define Variables
			var $displayTab = $(evt.target).parents('.datasets2tools-browsebar');
	...
	main: function(cannedAnalysisData) {
		...
		// Toggle Go Back Link Listener
		self.clickGoBackLink(cannedAnalysisData);
	}
};
...
////////////////////////////////////////////////////////////
///// 3.1 lincsBrowseTable /////////////////////////////////
////////////////////////////////////////////////////////////

var lincsBrowseTable = {

	/////////////////////////////////
	////// 2.2. toolInfo
	/////////////////////////////////

	toolInfo: function(toolMetadata) {

		// Define variables
		var interfaceHTML = '';

		// Add Go Back
		interfaceHTML += '<a class="datasets2tools-lincs-goback-link"><<<&nbsp&nbspBack to Analysis Tools</a><br>';

		// Add Tool Icon and Name
		interfaceHTML += '<div class="datasets2tools-lincs-toolinfo-tab"><img class="datasets2tools-toolinfo-tool-img" src="' + toolMetadata['tool_icon_url'] + '"><div class="datasets2tools-toolname-label">' + toolMetadata['tool_name'] + '</div></div>';

		// Add Tool Description
		interfaceHTML += '<p class="datasets2tools-browsebar-label">Tool Information</p><p class="datasets2tools-browsebar-text">' + toolMetadata['tool_description'] + '</p>';

		// Add Table Description
		interfaceHTML += '<p class="datasets2tools-browsebar-label">Canned Analyses</p><p class="datasets2tools-browsebar-text"> Below are the canned analyses associated tot he specified tool and dataset.  The search bar allows to restrict query results.   The user can input custom metadata fields. </p>';
	...
	browseTable: function(pairCannedAnalyses, toolIconUrl, pageNr=1) {

		// Get browse table
		var browseTableHTML = '<div class="datasets2tools-browse-bar">' + prepareBrowseTable(pairCannedAnalyses, toolIconUrl, pageNr, 150) + '</div>';
	...
	prepare: function(cannedAnalysisData, toolId) {

		// Define variables
		var self = this,
			toolMetadata = cannedAnalysisData['tools'][toolId],
			datasetAccession = Object.keys(cannedAnalysisData['canned_analyses'])[0],
			...
			interfaceHTML = self.toolInfo(toolMetadata) + self.browseTable(pairCannedAnalyses, toolIconUrl);
